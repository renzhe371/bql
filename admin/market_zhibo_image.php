...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		global $db_market_zhibo_image;
		parent::__construct($server,$request);
		
		$this->table=$db_market_zhibo_image;
		$this->table_name=_TABLE."zhibo_image";
	...
	function SEARCH_PAGE($form){
		...
		$this->template_name="market_zhibo_image.tpl";
	}
	...
	function formPage($form)
	{
		...
		if(!empty($form["image_path2"]))
		{
			...
			{
				$form['sizeArr2']=$this->myGetImgSize($form["image_path2"],_IMAGE_S_WIDTH,_IMAGE_S_HEIGHT);
			}
			else 
			{
				$form["image_path2"]="";
		...
		$this->template_name="zhibo_image_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			...
			{
				$file_name="vs".$form['image_id'].$this->getExtension($form["image_path2"]);
				copy($form["image_path2"],$file_path.$file_name);
				$this->imgScaling($file_path.$file_name,$file_path,"vs_".$file_name,_AMDIN_IMAGE_WIDTH,_AMDIN_IMAGE_HEIGHT);
			...
			$this->location ( "market_zhibo_image.php?mode=PAGELIST" );
		}	
	}
	...
	function checkSave($table,&$form)
	{
		...
		if($_FILES["img_upload2"]['name']!='')
		{
			...
			{
				$name=_TMP_FILE_DIR."vr_".date("YmdHis").$this->getExtension($_FILES["img_upload2"]['name']);
				if(!file_exists(_TMP_FILE_DIR))
					mkdir(_TMP_FILE_DIR,0777);
