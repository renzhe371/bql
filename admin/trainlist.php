...
class thisPage extends MyWeb {
	...
	function SEARCH_PAGE($form){
		...
		$this->template_name="train_list.tpl";
	}
	...
	function EDIT($form) {
		...
		{
			$form["train_id"]=$form["select_id"][0];
		}
		if(!empty($form["train_id"]))
		{
			$dbclass=$this->getDBInstance();
			$sql="select * from ".$this->table_name;
			$sql.=" where train_id = '".$form["train_id"]."'";
	...
	function formPage($form)
	{
		...
		$this->template_name="train_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			if($form["add_flg"]=="1")
			{
				$sql="select max(train_id) from ".$this->table_name;
				$max_id = $dbclass->GetOne($sql);
			    $form['train_id'] =$max_id+1;
				$dbclass->SQLInsert ( $this->table_name, $form, array_keys($this->table) );
			}
			else 
			{
				$dbclass->SQLUpdate ( $this->table_name, $form, array_keys($this->table),"train_id='".$form['train_id']."'" );
			}
			$this->location ( "trainlist.php" );
		}	
	}
	...
	function DEL($form)
	{
		if(!empty($form["train_id"]))
		{
			$form ["select_id"][0]=$form["train_id"];
		}
		if (! empty ( $form ["select_id"] )&&is_array($form ["select_id"])) {
			$dbclass=$this->getDBInstance();
			$where = " train_id in ('".implode("','",$form ["select_id"])."') ";
			$dbclass->SQLDelete($this->table_name,$where);
			$dbclass->SQLUpdate ( _TABLE."train", array("train_id"=>""), array("train_id"),$where );
		}
		$this->location ( "trainlist.php" );
