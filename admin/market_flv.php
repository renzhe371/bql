...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		global $db_market_flv;
		parent::__construct($server,$request);
		
		$this->table=$db_market_flv;
		$this->table_name=_TABLE."market_flv";
	...
	function SEARCH_PAGE($form){
		...
		$sql="SELECT count(flv_id) FROM ".$this->table_name;
		$sql.=$where;
		...
		$this->template_name="market_flv.tpl";
	}
	...
	function EDIT($form) {
		...
		{
			$form["flv_id"]=$form["select_id"][0];
		}
		if(!empty($form["flv_id"]))
		{
			$dbclass=$this->getDBInstance();
			$sql="select * from ".$this->table_name;
			$sql.=" where flv_id = '".$form["flv_id"]."'";
	...
	function formPage($form)
	{
		...
		if(!empty($form["image_path"]))
		{
			...
			{
				$form['sizeArr']=$this->myGetImgSize($form["image_path"],_AMDIN_FLV_WIDTH,_AMDIN_FLV_HEIGHT);
			}
			else 
			{
				$form["image_path"]="";
		...
		$this->template_name="flv_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			if($form["add_flg"]=="1")
			{
				$sql="select max(flv_id) from ".$this->table_name;
				$max_id = $dbclass->GetOne($sql);
			    $form['flv_id'] =$max_id+1;
			}
			
			if(!file_exists(_UPLOAD_DIR_FLV))
				mkdir(_UPLOAD_DIR_FLV,0777);
			
			$file_path=_UPLOAD_DIR_FLV.$form['flv_id']."/";
			...
			{
				$file_name=$form['flv_id'].$this->getExtension($form["image_path"]);
				copy($form["image_path"],$file_path.$file_name);
				$this->imgScaling($file_path.$file_name,$file_path,"s_".$file_name,_FLV_S_WIDTH,_FLV_S_HEIGHT);
			...
			else 
			{
				...
				$dbclass->SQLUpdate ( $this->table_name, $form, array_keys($this->table),"flv_id='".$form['flv_id']."'" );
			}
			
			$this->location ( "market_flv.php?mode=PAGELIST" );
		}	
	}
	...
	function checkSave($table,&$form)
	{
		...
		else 
		{
			if($_FILES["img_upload"]['name']!='')
			{
				...
				{
					$name=_TMP_FILE_DIR."flv_".date("YmdHis").$this->getExtension($_FILES["img_upload"]['name']);
		...
		if($_FILES["video_file"]['name']!='')
		{
			...
			{
				$error["flv_news"]=ERROR_FILE_FAIL;
			}
			else 
			{
