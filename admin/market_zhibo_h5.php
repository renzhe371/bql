...
class thisPage extends MyWeb {
	...
	function SEARCH_PAGE($form){
		...
		$this->template_name="zhibo_h5.tpl";
	}
	...
	function EDIT($form) {
		...
		{
			$form["img_id"]=$form["select_id"][0];
		}
		if(!empty($form["img_id"]))
		{
			$dbclass=$this->getDBInstance();
			$sql="select * from ".$this->table_name;
			$sql.=" where img_id = '".$form["img_id"]."'";
	...
	{
		$smarty = &$this->getSmartyInstance();
		if(!empty($form["img_path"]))
		{
			if(file_exists($form["img_path"]))
			{
				$form['sizeArr']=$this->myGetImgSize($form["img_path"],_H5_S_WIDTH,_H5_S_HEIGHT);
			}
			else 
			{
				$form["img_path"]="";
		...
		if(!empty($form["image_path2"]))
		{
			...
			{
				$form['sizeArr2']=$this->myGetImgSize($form["image_path2"],_H5_S_WIDTH,_H5_S_HEIGHT);
			}
			else 
			{
				$form["image_path2"]="";
		...
		$this->template_name="zhibo_h5_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			if($form["add_flg"]=="1")
			{
				$sql="select max(img_id) from ".$this->table_name;
				$max_id = $dbclass->GetOne($sql);
			    $form['img_id'] =$max_id+1;
			}
			
			if(!file_exists(_UPLOAD_DIR_H5))
				mkdir(_UPLOAD_DIR_H5,0777);
			
			$file_path=_UPLOAD_DIR_H5.$form['img_id']."/";
			if(!file_exists($file_path))
				mkdir($file_path,0777);
			
			//上传图片
			if(!empty($form["img_path"])&&strpos($form["img_path"],_TMP_FILE_DIR) !== false)
			{
				$file_name=$form['img_id'].$this->getExtension($form["img_path"]);
				copy($form["img_path"],$file_path.$file_name);
				$this->imgScaling($file_path.$file_name,$file_path,"h_".$file_name,_AMDIN_H5_WIDTH,_AMDIN_H5_HEIGHT);
				$this->DelOneFile($form["img_path"]);
				$form["img_path"]=$file_path.$file_name;
			}
			
			if(!empty($form["image_path2"])&&strpos($form["image_path2"],_TMP_FILE_DIR) !== false)
			{
				$file_name="H_".$form['img_id'].$this->getExtension($form["image_path2"]);
			...
			{
				$dbclass->SQLUpdate ( $this->table_name, $form, array_keys($this->table),"img_id='".$form['img_id']."'" );
			}
			
			$this->location ( "market_zhibo_h5.php?mode=PAGELIST" );
		}	
	}
	...
	function checkSave($table,&$form)
	{
		...
		if($_FILES["img_upload"]['name']==''&&@$form["img_path"]=="")
		{
			$error["img_path"]=ERROR_IMAGE_NULL;
		}
		else 
		{
			if($_FILES["img_upload"]['name']!='')
			{
				...
				{
					$error["img_path"]=$table["img_path"]["text"].ERROR_IMAGE_UPLOAD;
				}
				elseif (strpos($_FILES["img_upload"]['type'],"image") === false)
				{
					$error["img_path"]=$table["img_path"]["text"].ERROR_IMAGE_INVALID;
				}
				else 
				{
					$name=_TMP_FILE_DIR."h5_".date("YmdHis").$this->getExtension($_FILES["img_upload"]['name']);
			...
			else 
			{
				if($_FILES["img_upload2"]['name']!='')
				{
					...
					{
						$name=_TMP_FILE_DIR."H5_".date("YmdHis").$this->getExtension($_FILES["img_upload2"]['name']);
						if(!file_exists(_TMP_FILE_DIR))
							mkdir(_TMP_FILE_DIR,0777);
	...
	function DEL($form)
	{
		if(!empty($form["img_id"]))
		{
			$form ["select_id"][0]=$form["img_id"];
		}
		if (! empty ( $form ["select_id"] )&&is_array($form ["select_id"])) {
			$dbclass=$this->getDBInstance();
			$where = " img_id in ('".implode("','",$form ["select_id"])."') ";
