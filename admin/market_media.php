...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		global $db_market_media;
		parent::__construct($server,$request);
		
		$this->table=$db_market_media;
		$this->table_name=_TABLE."media";
	...
	function SEARCH_PAGE($form){
		...
		$sql="SELECT count(media_id) FROM ".$this->table_name;
		$sql.=$where;
		...
		//$sql.=" as p,pro_media_category as c where p.category_id=c.category_id";
		...
		$this->template_name="market_media.tpl";
	}
	...
	function EDIT($form) {
		...
		{
			$form["media_id"]=$form["select_id"][0];
		}
		if(!empty($form["media_id"]))
		{
			$dbclass=$this->getDBInstance();
			$sql="select * from ".$this->table_name;
			$sql.=" where media_id = '".$form["media_id"]."'";
	...
	function formPage($form)
	{
		...
		if(!empty($form["image_path"]))
		{
			...
			{
				$form['sizeArr']=$this->myGetImgSize($form["image_path"],_MEDIA_S_WIDTH,_MEDIA_S_HEIGHT);
			}
			else 
			{
				$form["image_path"]="";
		...
		$this->template_name="media_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			if($form["add_flg"]=="1")
			{
				$sql="select max(media_id) from ".$this->table_name;
				$max_id = $dbclass->GetOne($sql);
			    $form['media_id'] =$max_id+1;
			}
			
			if(!file_exists(_UPLOAD_DIR_MEDIA))
				mkdir(_UPLOAD_DIR_MEDIA,0777);
			
			$file_path=_UPLOAD_DIR_MEDIA.$form['media_id']."/";
			...
			{
				$file_name=$form['media_id'].$this->getExtension($form["image_path"]);
				copy($form["image_path"],$file_path.$file_name);
				$this->imgScaling($file_path.$file_name,$file_path,"s_".$file_name,_AMDIN_MEDIA_WIDTH,_AMDIN_MEDIA_HEIGHT);
			...
			else 
			{
				...
				$dbclass->SQLUpdate ( $this->table_name, $form, array_keys($this->table),"media_id='".$form['media_id']."'" );
			}
			
			$this->location ( "market_media.php?mode=PAGELIST" );
		}	
	}
	...
	function checkSave($table,&$form)
	{
		...
		else 
		{
			if($_FILES["img_upload"]['name']!='')
			{
				...
				{
					$name=_TMP_FILE_DIR."media_".date("YmdHis").$this->getExtension($_FILES["img_upload"]['name']);
		...
		if($_FILES["video_file"]['name']!='')
		{
			...
			{
				$error["media_news"]=ERROR_FILE_FAIL;
			}
			else 
			{
				...
				$form["media_news"]=$name;
			}
		}
		
		if($_FILES["video_file2"]['name']!='')
		{
			if($_FILES["video_file2"]['error']!='0')
			{
				$error["media_mp4"]=ERROR_FILE_FAIL;
			}
			elseif (strtolower($this->getExtension($_FILES["video_file2"]['name']))!=".mp4")
            {
                $error["video_file2"]=ERROR_INVALID;
            }
			else 
			{
				$name=_TMP_FILE_DIR_FLV."mbql_".date("YmdHis").$this->getExtension($_FILES["video_file2"]['name']);
				if(!file_exists(_TMP_FILE_DIR_FLV))
					mkdir(_TMP_FILE_DIR_FLV,0777);
