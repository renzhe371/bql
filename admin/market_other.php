...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		global $db_market_other;
		parent::__construct($server,$request);
		
		$this->table=$db_market_other;
		$this->table_name=_TABLE."market_other";
	...
	function SEARCH_PAGE($form){
		...
		$sql="SELECT count(other_id) FROM ".$this->table_name;
		$sql.=$where;
		...
		$this->template_name="market_other.tpl";
	}
	...
	function EDIT($form) {
		...
		{
			$form["other_id"]=$form["select_id"][0];
		}
		if(!empty($form["other_id"]))
		{
			$dbclass=$this->getDBInstance();
			$sql="select * from ".$this->table_name;
			$sql.=" where other_id = '".$form["other_id"]."'";
	...
	function formPage($form)
	{
		...
		$this->template_name="other_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			if($form["add_flg"]=="1")
			{
				$sql="select max(other_id) from ".$this->table_name;
				$max_id = $dbclass->GetOne($sql);
			    $form['other_id'] =$max_id+1;
			}
			
			if(!file_exists(_UPLOAD_DIR_OTHER))
				mkdir(_UPLOAD_DIR_OTHER,0777);
			
			$file_path=_UPLOAD_DIR_OTHER.$form['other_id']."/";
			...
			else 
			{
				...
				$dbclass->SQLUpdate ( $this->table_name, $form, array_keys($this->table),"other_id='".$form['other_id']."'" );
			}
			
			$this->location ( "market_other.php?mode=PAGELIST" );
		}	
	}
	...
	function checkSave($table,&$form)
	{
		...
		else 
		{
			if($_FILES["img_upload"]['name']!='')
			{
				...
				{
					$name=_TMP_FILE_DIR."other_".date("YmdHis").$this->getExtension($_FILES["img_upload"]['name']);
