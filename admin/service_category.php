...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		global $db_service_category;
		parent::__construct($server,$request);
		
		$this->table=$db_service_category;
		$this->table_name=_TABLE."service_category";
	...
	function SEARCH_PAGE($form){
		...
		$this->template_name="service_category_list.tpl";
	}
	...
	function formPage($form)
	{
		...
		$this->template_name="service_category_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			...
			$this->location ( "service_category.php" );
		}	
	}
	...
	function DEL($form)
	{
		...
		if (! empty ( $form ["select_id"] )&&is_array($form ["select_id"])) {
			...
			$dbclass->SQLUpdate ( _TABLE."service", array("category_id"=>""), array("category_id"),$where );
		}
		$this->location ( "service_category.php" );
