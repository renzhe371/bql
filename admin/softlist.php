...
class thisPage extends MyWeb {
	...
	function SEARCH_PAGE($form){
		...
		$sql="SELECT soft_id,soft_name,soft_cid,soft_url,order_num,update_time FROM ".$this->table_name;
		$sql.=$where;
		...
		$this->template_name="soft_list.tpl";
	}
	...
	{
		$smarty = &$this->getSmartyInstance();
		if(!empty($form["soft_img"]))
		{
			if(file_exists($form["soft_img"]))
			{
				$form['sizeArr']=$this->myGetImgSize($form["soft_img"],_ADMIN_SOFT_WIDTH,_ADMIN_SOFT_HEIGHT);
			}
			else 
			{
				$form["soft_img"]="";
		...
		$this->template_name="soft_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			if($form["add_flg"]=="1")
			{
				$sql="select max(soft_id) from ".$this->table_name;
				$max_id = $dbclass->GetOne($sql);
			    $form['soft_id'] =$max_id+1;
			}
			
			
			//上传图片
			if(!file_exists(_UPLOAD_DIR_SOFT))
				mkdir(_UPLOAD_DIR_SOFT,0777);
			
			$file_path=_UPLOAD_DIR_SOFT.$form['soft_id']."/";
			if(!file_exists($file_path))
				mkdir($file_path,0777);
				chmod($file_path,0777);

			if(!empty($form["soft_img"])&&strpos($form["soft_img"],_TMP_FILE_DIR) !== false)
			{
				$file_name=$form['soft_id'].$this->getExtension($form["soft_img"]);
				copy($form["soft_img"],$file_path.$file_name);
				$this->imgScaling($file_path.$file_name,$file_path,"s_".$file_name,_SOFT_S_WIDTH,_SOFT_S_HEIGHT);
				$this->DelOneFile($form["soft_img"]);
				$form["soft_img"]=$file_path.$file_name;
			}
			...
			else 
			{
				...
				$dbclass->SQLUpdate ( $this->table_name, $form, array_keys($this->table),"soft_id='".$form['soft_id']."'" );
			}
			
			$this->location ( "softlist.php?mode=PAGELIST" );
		}	
	}
	...
	function checkSave($table,&$form)
	{
		...
		if($_FILES["img_upload"]['name']==''&&@$form["soft_img"]=="")
		{
			$error["soft_img"]=ERROR_IMAGE_NULL;
		}
		else 
		{
			if($_FILES["img_upload"]['name']!='')
			{
				...
				{
					$error["soft_img"]=$table["soft_img"]["text"].ERROR_IMAGE_UPLOAD;
				}
				elseif (strpos($_FILES["img_upload"]['type'],"image") === false)
				{
					$error["soft_img"]=$table["soft_img"]["text"].ERROR_IMAGE_INVALID;
				}
				else 
				{
					$name=_TMP_FILE_DIR."soft_".date("YmdHis").$this->getExtension($_FILES["img_upload"]['name']);
			 ...
			 if($_FILES["pdf_upload"]['name']!=''){
				...
				if($_FILES["pdf_upload"]['error']!='0'){
					...
					$error["soft_url"]=ERROR_FILE_FAIL;
				}else{
				
					$name=_TMP_FILE_DIR."pdf_".date("YmdHis").$this->getExtension($_FILES["pdf_upload"]['name']);
					if(!file_exists(_TMP_FILE_DIR))
						mkdir(_TMP_FILE_DIR,0777);
					move_uploaded_file($_FILES["pdf_upload"]['tmp_name'] , $name);
					$form["soft_url"]=$name;
				}
			}
		}
