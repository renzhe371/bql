...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		global $db_djs_category;
		parent::__construct($server,$request);
		
		$this->table=$db_djs_category;
		$this->table_name=_TABLE."djs_category";
	...
	function SEARCH_PAGE($form){
		...
		$sql="SELECT category_id,category_name,order_num FROM ".$this->table_name;
		$sql.=$where;
		...
		$this->template_name="djs_category_list.tpl";
	}
	...
	function formPage($form)
	{
		...
		$this->template_name="djs_category_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			...
			$this->location ( "djs_category.php" );
		}	
	}
	...
	function DEL($form)
	{
		...
		if (! empty ( $form ["select_id"] )&&is_array($form ["select_id"])) {
			...
			$dbclass->SQLUpdate ( _TABLE."djs", array("category_id"=>""), array("category_id"),$where );
		}
		$this->location ( "djs_category.php" );
