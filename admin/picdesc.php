...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		...
		$this->table=$db_picdesc;
		$this->table_name=_TABLE."picdesc";
	...
	function SEARCH_PAGE($form){
		...
		$sql="SELECT count(desc_id) FROM ".$this->table_name;
		$sql.=$where;
		...
		$this->template_name="picdesc.tpl";
	}
	...
	function EDIT($form) {
		...
		{
			$form["desc_id"]=$form["select_id"][0];
		}
		if(!empty($form["desc_id"]))
		{
			$dbclass=$this->getDBInstance();
			$sql="select * from ".$this->table_name;
			$sql.=" where desc_id = '".$form["desc_id"]."'";
	...
	function formPage($form)
	{
		...
		$this->template_name="picdesc_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			if($form["add_flg"]=="1")
			{
				$sql="select max(desc_id) from ".$this->table_name;
				$max_id = $dbclass->GetOne($sql);
			    $form['desc_id'] =$max_id+1;
			}
			
			if(!file_exists(_UPLOAD_DIR_PICDESC))
				mkdir(_UPLOAD_DIR_PICDESC,0777);
			
			$file_path=_UPLOAD_DIR_PICDESC.$form['desc_id']."/";
			...
			{
				$file_name=$form['desc_id'].$this->getExtension($form["image_path"]);
			...
			else 
			{
				...
				$dbclass->SQLUpdate ( $this->table_name, $form, array_keys($this->table),"desc_id='".$form['desc_id']."'" );
			}
			
			$this->location ( "picdesc.php?mode=PAGELIST" );
		}	
	}
	...
	function DEL($form)
	{
		if(!empty($form["desc_id"]))
		{
			$form ["select_id"][0]=$form["desc_id"];
		}
		if (! empty ( $form ["select_id"] )&&is_array($form ["select_id"])) {
			$dbclass=$this->getDBInstance();
			$where = " desc_id in ('".implode("','",$form ["select_id"])."') ";
