...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		global $db_case_category;
		parent::__construct($server,$request);
		
		$this->table=$db_case_category;
		$this->table_name=_TABLE."case_category";
	...
	function SEARCH_PAGE($form){
		...
		$this->template_name="case_category_list.tpl";
	}
	...
	function formPage($form)
	{
		...
		$this->template_name="case_category_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			...
			$this->location ( "case_category.php" );
		}	
	}
	...
	function DEL($form)
	{
		...
		if (! empty ( $form ["select_id"] )&&is_array($form ["select_id"])) {
			...
			$dbclass->SQLUpdate ( _TABLE."case", array("category_id"=>""), array("category_id"),$where );
		}
		$this->location ( "case_category.php" );
