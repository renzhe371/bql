...
class thisPage extends MyWeb {
	...
	function SEARCH_PAGE($form){
		...
		$sql="SELECT count(case_id) FROM ".$this->table_name;
		$sql.=$where;
		...
		$sql="SELECT case_id,case_name,case_name_en,category_id,order_num,update_time FROM ".$this->table_name;
		$sql.=$where;
		...
		$this->template_name="case_list.tpl";
	}
	...
	function EDIT($form) {
		...
		{
			$form["case_id"]=$form["select_id"][0];
		}
		if(!empty($form["case_id"]))
		{
			$dbclass=$this->getDBInstance();
			$sql="select * from ".$this->table_name;
			$sql.=" where case_id = '".$form["case_id"]."'";
	...
	function formPage($form)
	{
		...
		if(!empty($form["image_path"]))
		{
			...
			{
				$form['sizeArr']=$this->myGetImgSize($form["image_path"],_AMDIN_CASE_WIDTH,_AMDIN_CASE_HEIGHT);
			}
			else 
			{
				$form["image_path"]="";
		...
		$this->template_name="case_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			if($form["add_flg"]=="1")
			{
				$sql="select max(case_id) from ".$this->table_name;
				$max_id = $dbclass->GetOne($sql);
			    $form['case_id'] =$max_id+1;
			}
			
			if(!file_exists(_UPLOAD_DIR_CASE))
				mkdir(_UPLOAD_DIR_CASE,0777);
			
			$file_path=_UPLOAD_DIR_CASE.$form['case_id']."/";
			...
			{
				$file_name=$form['case_id'].$this->getExtension($form["image_path"]);
				copy($form["image_path"],$file_path.$file_name);
				$this->imgScaling($file_path.$file_name,$file_path,"s_".$file_name,_CASE_S_WIDTH,_CASE_S_HEIGHT);
			...
			else 
			{
				...
				$dbclass->SQLUpdate ( $this->table_name, $form, array_keys($this->table),"case_id='".$form['case_id']."'" );
			}
			
			$this->location ( "case.php?mode=PAGELIST" );
		}	
	}
	...
	function checkSave($table,&$form)
	{
		...
		else 
		{
			if($_FILES["img_upload"]['name']!='')
			{
				...
				{
					$name=_TMP_FILE_DIR."case_".date("YmdHis").$this->getExtension($_FILES["img_upload"]['name']);
		...
		if(empty($error["case_name"])&&!empty($form["case_name"]))
		{
			$sql="select case_id from ".$this->table_name;
			$sql.=" where case_name = '".$form["case_name"]."'";
			if(!empty($form["case_id"])&&@$form["add_flg"]=="0")
			{
				$sql.=" and case_id <> '".$form["case_id"]."'";
			}
			$rec=$dbclass->GetOne($sql);
			if(!empty($rec))
			{
				$error["case_name"]=str_replace("%",$table["case_name"]["text"],ERROR_HAVE_EXIST);
			}
		}
