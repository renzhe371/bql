...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		global $db_market_live;
		parent::__construct($server,$request);
		
		$this->table=$db_market_live;
		$this->table_name=_TABLE."live_image";
	...
	function SEARCH_PAGE($form){
		...
		$this->template_name="market_live.tpl";
	}
	...
	function formPage($form)
	{
		...
		if(!empty($form["image_path"]))
		{
			...
			{
				$form['sizeArr']=$this->myGetImgSize($form["image_path"],_LIVE_S_WIDTH,_LIVE_S_HEIGHT);
			}
			else 
			{
				$form["image_path"]="";
		...
		$this->template_name="live_edit.tpl";
	}
	...
	function SAVE($form) {
		...
		else 
		{
			...
			if(!file_exists(_UPLOAD_DIR_LIVE))
				mkdir(_UPLOAD_DIR_LIVE,0777);
			
			$file_path=_UPLOAD_DIR_LIVE.$form['image_id']."/";
			...
			if(!empty($form["image_path"])&&strpos($form["image_path"],_TMP_FILE_DIR) !== false)
			{
				...
				$this->imgScaling($file_path.$file_name,$file_path,"s_".$file_name,_AMDIN_LIVE_WIDTH,_AMDIN_LIVE_HEIGHT);
			...
			$this->location ( "market_live.php?mode=PAGELIST" );
		}	
	}
	...
	function DEL($form)
	{
		...
		if (! empty ( $form ["select_id"] )&&is_array($form ["select_id"])) {
			...
			{
				$this->DelDir(_UPLOAD_DIR_LIVE.$id."/");
			}
		}
		$this->location ( "market_live.php?mode=PAGELIST" );
