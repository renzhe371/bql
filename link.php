...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
		parent::__construct($server,$request); $this->table_name=_TABLE."active";
	...
	function IMDEX($form){
		...
		$sql="select active_title,active_content from ".$this->table_name;
		$sql.=" where active_id=".$form['tid'];
		$link_content=$dbclass->GetAll($sql);
		$smarty->assign("link_content",$link_content[0]);
