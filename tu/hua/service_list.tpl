<div id="main_body">
<form id="myform" name="myform" action="service.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="44%"><a href="javascript:setOrderBy('service_title <{if $activeField eq 'service_title'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.service_title.text}></a><{if $ordField eq "service_title desc"}>↑<{/if}><{if $ordField eq "service_title asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td><input type="checkbox" name="select_id[]" value="<{$list[loop].service_id}>"></td>
            <td class="td_left"><{$list[loop].service_title}></td>
          	<td class="td_left"><{if $list[loop].category_id eq '' or $list[loop].category_id eq '0'}>未设置<{else}><{$list[loop].category_id|getServiceCategory}><{/if}></td>
          	<td class="td_center"><{$list[loop].update_time}></td>
          	<td class="td_center"><{$list[loop].order_num}></td>
            <td class="td_center">
            	<a href="?mode=EDIT&service_id=<{$list[loop].service_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&service_id=<{$list[loop].service_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
