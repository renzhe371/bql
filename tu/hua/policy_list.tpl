<div id="main_body">
<form id="myform" name="myform" action="policylist.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="20%"><a href="javascript:setOrderBy('name <{if $activeField eq 'name'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.name.text}></a><{if $ordField eq "name desc"}>↑<{/if}><{if $ordField eq "name asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            ...
            <td><{if $list[loop].category_id eq '' or $list[loop].category_id eq '0'}>未设置<{else}><{$list[loop].category_id|getCorpCategory}><{/if}></td>
