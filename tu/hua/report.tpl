<div id="main_body">
<form id="myform" name="myform" action="report.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="44%"><a href="javascript:setOrderBy('report_title <{if $activeField eq 'report_title'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.report_title.text}></a><{if $ordField eq "report_title desc"}>↑<{/if}><{if $ordField eq "report_title asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td><input type="checkbox" name="select_id[]" value="<{$list[loop].report_id}>"></td>
            <td class="td_left"><{$list[loop].report_title}></td>
          	<td class="td_center"><{$list[loop].update_time}></td>
          	<td class="td_center"><{$list[loop].order_num}></td>
            <td class="td_center">
            	<a href="?mode=EDIT&report_id=<{$list[loop].report_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&report_id=<{$list[loop].report_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
