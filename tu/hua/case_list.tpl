<div id="main_body">
<form id="myform" name="myform" action="case.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="38%"><a href="javascript:setOrderBy('case_name <{if $activeField eq 'case_name'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.case_name.text}></a><{if $ordField eq "case_name desc"}>↑<{/if}><{if $ordField eq "case_name asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td class="td_center"><input type="checkbox" name="select_id[]" value="<{$list[loop].case_id}>"></td>
            <td><{$list[loop].case_name}></td>
            <td><{if $list[loop].category_id eq '' or $list[loop].category_id eq '0'}>未设置<{else}><{$list[loop].category_id|getCaseCategory}><{/if}></td>
            <td><{$list[loop].update_time}></td>
            <td><{$list[loop].order_num}></td>
            <td class="td_center">
            	<a href="?mode=EDIT&case_id=<{$list[loop].case_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&case_id=<{$list[loop].case_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
