<div id="main_body">
<form id="myform" name="myform" action="market_zhibo.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="30%"><a href="javascript:setOrderBy('zb_title <{if $activeField eq 'zb_title'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.zb_title.text}></a><{if $ordField eq "zb_title desc"}>↑<{/if}><{if $ordField eq "zb_title asc"}>↓<{/if}></th>
			<th width="10%"><a href="javascript:setOrderBy('zb_status <{if $activeField eq 'zb_status'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.zb_status.text}></a><{if $ordField eq "zb_status desc"}>↑<{/if}><{if $ordField eq "zb_status asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td class="td_center"><input type="checkbox" name="select_id[]" value="<{$list[loop].zb_id}>"></td>
            <td><{$list[loop].zb_title}></td>
			<td><{$list[loop].zb_status}></td>
            ...
            <td class="td_center">
            	<a href="?mode=EDIT&zb_id=<{$list[loop].zb_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&zb_id=<{$list[loop].zb_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
