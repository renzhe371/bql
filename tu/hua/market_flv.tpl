<div id="main_body">
<form id="myform" name="myform" action="market_flv.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="10%"><a href="javascript:setOrderBy('flv_id <{if $activeField eq 'flv_id'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.flv_id.text}></a><{if $ordField eq "flv_id desc"}>↑<{/if}><{if $ordField eq "flv_id asc"}>↓<{/if}></th>
            <th width="26%"><a href="javascript:setOrderBy('flv_name <{if $activeField eq 'flv_name'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.flv_name.text}></a><{if $ordField eq "flv_name desc"}>↑<{/if}><{if $ordField eq "flv_name asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td class="td_center"><input type="checkbox" name="select_id[]" value="<{$list[loop].flv_id}>"></td>
            <td><{$list[loop].flv_id}></td>
            <td><{$list[loop].flv_name}></td>
            ...
            <td class="td_center">
            	<a href="?mode=EDIT&flv_id=<{$list[loop].flv_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&flv_id=<{$list[loop].flv_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
