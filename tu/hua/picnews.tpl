<div id="main_body">
<form id="myform" name="myform" action="picnews.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="10%"><a href="javascript:setOrderBy('image_id <{if $activeField eq 'image_id'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.image_id.text}></a><{if $ordField eq "image_id desc"}>↑<{/if}><{if $ordField eq "image_id asc"}>↓<{/if}></th>
            <th width="26%"><a href="javascript:setOrderBy('image_name <{if $activeField eq 'image_name'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.image_name.text}></a><{if $ordField eq "image_name desc"}>↑<{/if}><{if $ordField eq "image_name asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td class="td_center"><input type="checkbox" name="select_id[]" value="<{$list[loop].image_id}>"></td>
            <td><{$list[loop].image_id}></td>
            <td><{$list[loop].image_name}></td>
            <td><{if $list[loop].category_id eq '' or $list[loop].category_id eq '0'}>未设置<{else}><{$list[loop].category_id|getPicCategory}><{/if}></td>
            <td><{$list[loop].update_time}></td>
            <td><{$list[loop].order_num}></td>
            <td class="td_center">
            	<a href="?mode=EDIT&image_id=<{$list[loop].image_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&image_id=<{$list[loop].image_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
