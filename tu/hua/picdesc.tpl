<div id="main_body">
<form id="myform" name="myform" action="picdesc.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="10%"><a href="javascript:setOrderBy('desc_id <{if $activeField eq 'desc_id'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.desc_id.text}></a><{if $ordField eq "desc_id desc"}>↑<{/if}><{if $ordField eq "desc_id asc"}>↓<{/if}></th>
            <th width="26%"><a href="javascript:setOrderBy('desc_name <{if $activeField eq 'desc_name'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.desc_name.text}></a><{if $ordField eq "desc_name desc"}>↑<{/if}><{if $ordField eq "desc_name asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td class="td_center"><input type="checkbox" name="select_id[]" value="<{$list[loop].desc_id}>"></td>
            <td><{$list[loop].desc_id}></td>
            <td><{$list[loop].desc_name}></td>
            <td><{if $list[loop].category_id eq '' or $list[loop].category_id eq '0'}>未设置<{else}><{$list[loop].category_id|getDescCategory}><{/if}></td>
            <td><{$list[loop].update_time}></td>
            <td><{$list[loop].order_num}></td>
            <td class="td_center">
            	<a href="?mode=EDIT&desc_id=<{$list[loop].desc_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&desc_id=<{$list[loop].desc_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
