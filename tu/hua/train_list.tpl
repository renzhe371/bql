      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="40%"><a href="javascript:setOrderBy('train_corp <{if $activeField eq 'train_corp'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.train_corp.text}></a><{if $ordField eq "train_corp desc"}>↑<{/if}><{if $ordField eq "train_corp asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td class="td_center"><input type="checkbox" name="select_id[]" value="<{$list[loop].train_id}>"></td>
            <td><{$list[loop].train_corp}></td>
            <td><{$list[loop].order_num}></td>
            <td class="td_center">
            	<a href="?mode=EDIT&train_id=<{$list[loop].train_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&train_id=<{$list[loop].train_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
