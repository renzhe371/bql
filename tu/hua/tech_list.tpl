<div id="main_body">
<form id="myform" name="myform" action="tech.php" method="POST">
      ...
      <{if $list|smarty:nodefaults|@count ne '0'}>
        <table cellpadding="0" cellspacing="0">
          <tr>
            ...
            <th width="44%"><a href="javascript:setOrderBy('tech_name <{if $activeField eq 'tech_name'}><{$ascdesc}><{else}><{$sortAD}><{/if}>');"><{$table.tech_name.text}></a><{if $ordField eq "tech_name desc"}>↑<{/if}><{if $ordField eq "tech_name asc"}>↓<{/if}></th>
          ...
          <tr <{if $smarty.section.loop.index%2 eq 1}>class="even"<{else}>class="odd"<{/if}>>
            <td><input type="checkbox" name="select_id[]" value="<{$list[loop].tech_id}>"></td>
            <td class="td_left"><{$list[loop].tech_name}></td>
          	<td class="td_center"><{$list[loop].update_time}></td>
          	<td class="td_center"><{$list[loop].order_num}></td>
            <td class="td_center">
            	<a href="?mode=EDIT&tech_id=<{$list[loop].tech_id}>"><img src="common/images/icons/pencil.png" alt="Edit" /></a> 
            	<a href="?mode=DEL&tech_id=<{$list[loop].tech_id}>" onclick="if(!delConfirm()) return false;"><img src="common/images/icons/cross.png" alt="Delete" /></a>
            </td>
          </tr>
          <{/section}>
        </table>
