<div class="about_navs">
	<ul class="max_w1750 m_auto min_w1200 po-r cf">
		...
		<li class="float-l">
			...
			<a href="CustomerList.html?category_id=<{$corp_list[loop].corp_id}>" <{if $smarty.section.loop.first}>class="cur"<{/if}>>
				<img src="<{$corp_list[loop].corp_logo}>">
			</a>
			<{else}>
			<a href="CustomerList.html?category_id=<{$corp_list[loop].corp_id}>" <{if $corp_list[loop].corp_id eq $form.category_id}>class="cur"<{/if}>>
				<img src="<{$corp_list[loop].corp_logo}>">
			</a>
			<{/if}>
		</li>
