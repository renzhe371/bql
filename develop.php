...
class thisPage extends MyWeb {
	...
	function thisPage(&$server,&$request) {
        ...
        $this->table_name=_TABLE."investor";
	...
	function IMDEX($form){
		...
		$sql="select develop from ".$this->table_name;
        $develop_content=$dbclass->GetOne($sql);
		$smarty->assign("develop_content",$develop_content);
		...
		$this->template_name="develop.tpl";
